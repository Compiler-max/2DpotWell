...
#read in raw data k space grid
#f1      	= open("rawData/kpts.dat",'rb')
#kpts    	= np.fromfile(f1,dtype='float64',count=-1)
#f1.close()
#
#f11      	= open("rawData/kptsW.dat",'rb')
#kptsW    	= np.fromfile(f11,dtype='float64',count=-1)
#f11.close()


#read in raw data energy parameters
#rawData = np.delete(rawData,0)#remove first 8bytes
...
#	ax.plot(kpts[:],En[:,n],'+-',color='k',markersize=0.8,linewidth=0.3) 
#
#for m in range(nWfs):
#	ax.plot(kptsW[:],EnW[:,m],'+-',color='red',markersize=0.8,linewidth=0.3)
#
#
##change x ticks to appropriate k points (this depends on the kpt path generated by kptsgen.pl in the "old way")
#xticks = np.array([k0,k0/2,0,-k0/2,-k0])				 #steps in kspace
#xtickLabel = np.array([r'$-\frac{\pi}{a}$',r'$-\frac{\pi}{2a}$',0,r'$\frac{\pi}{2a}$',r'$\frac{\pi}{a}$']) #symmetry points visited
#ax.set_xticks(xticks)	
#ax.set_xticklabels(xtickLabel,fontsize=14)
#ax.set_xlim([k0,-k0])
#
...
#
##plot aesthetics
#plt.title('1D chain',fontsize=18)
...
'''
#get the meshes
fq          = open(search_dir+'/'+'qpts.dat','rb')
qpts        = np.fromfile(fq,dtype='float64',count=-1)
fq.close()
nQ          = int(qpts.size / dim)
qpts        = np.reshape(qpts,(nQ,dim))

#fk         = open(search_dir+'/'+'qpts.dat','rb')
#kpts       = np.fromfile(fk,dtype='float64',count=-1)
#fk.close()
#nK         = int(kpts.size / dim)
#kpts       = np.reshape(kpts,(nK,dim))

#try to get nSolve
nSolve = 0
for file in os.listdir(search_dir):
    if fnmatch.fnmatch(file,'enK.0000001'):
            f       = open(search_dir+'/'+file,'rb')
            tmp     = np.fromfile(f,dtype='float64',count=-1)
            nSolve  = tmp.size
...
print('*')
print('*')
print('********read the energies:********************************')


for fname in os.listdir(search_dir):
    if fnmatch.fnmatch(fname, 'enK.*'):
        qpt_int = int(float(fname[5:len(fname)]))

        f_path = search_dir+'/'+fname
        f_open          = open(f_path,'rb') 
        tmp = np.fromfile(f_open,dtype='float64',count=-1)
        f_open.close()
        
        if tmp.size is nSolve:
            en_ABIN[:,qpt_int-1] = tmp
        else:
            print('warning found ',tmp.size,' states at qpt=',qpt_int,'! expected where',nSolve,' states')
        
        print('read qpt_int=',qpt_int)

en_ABIN = en_ABIN * au_to_ev
