...
#plt.subplot(111)
	...
	if plot_titles:
		if do_plot_percent:
			plt.title(r'magnetoel. x shift',fontsize=title_size)
		else:
	...
	cb1 = fig.colorbar(CSx_tot, cax=cax, cmap=cmap, label=r'polarizability $(p_{\mathrm{q}})$')
	...
	if plot_titles:
		if do_plot_percent:
			plt.title('magnetoel. y shift',fontsize=title_size)
		else:
			plt.title('magnetoel. y shift',fontsize=title_size)
	if plot_k_labels:
		plt.ylabel('ky (a.u.)',fontsize=k_label_size)
	...
	plt.tight_layout(pad=1.25,rect=(0,0,scale,scale))
	cax = fig.add_axes([.81, 0.1, .03, scale*0.8])	# [left, bottom, width, height] 
	cb1 = fig.colorbar(CSy_tot, cax=cax, cmap=cmap, label=r'polarizability $(p_{\mathrm{q}})$')
	#norm = mpl.colors.Normalize(vmin=zmin, vmax=zmax)
	#cb1 = mpl.colorbar.ColorbarBase(cax, cmap=cmap,
    #                            norm=norm,
    #                            orientation='vertical')
...
def plotDir():
	...
	for plotState in range(1,nwfs+1):
		...
		plotNiu_orb(searchDir,  plotState, aX_ang=aX,aY_ang=aY, plot_percent=True, plot_essin=False, plot_titles=True)
