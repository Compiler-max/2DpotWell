...
#data to calculate
Rashba=( -2.0 0.0 1.0 1.5 2.0 2.5 3.0) #10.0 100.0 1000.0 )
...
#body
for aRash in ${Rashba[*]}; do
	#
	echo '['$(date +"%T")']: start aRashba='$aRash'eV ang; '
	#
	#creates directories
	dir=$label'/a'$aRash
	if [[ ! -e $dir ]]; then
	...
	cd $dir
	sed -i "/aRashba/c\    aRashba  =  $aRash " ./input.txt
	wait
	#
	#execute calculation
	./subStream.sh $nProcs
