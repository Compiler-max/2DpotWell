module effTB
use omp_lib
	use mathematics,	only:	dp, PI_dp, i_dp, acc, myExp, myLeviCivita, nIntegrate, isHermitian
	use sysPara
	...
	subroutine TBviaKspace(unkW, EnH, Uq, tHopp, rHopp)
		complex(dp),		intent(in)		:: unkW(:,:,:), Uq(:,:,:)
		real(dp),			intent(in)		:: EnH(:,:)
		real(dp),			intent(out)		:: tHopp(:,:,:), rHopp(:,:,:,:)
		...
		do R = 1, nSC
			do qi = 1, nQ
				...
				!
				!RHOPP
				rImag(1,:,:,R)	= rImag(1,:,:,R) 	+ phase * AWcoarse(1,:,:,qi) 
				rImag(2,:,:,R)	= rImag(2,:,:,R) 	+ phase * AWcoarse(2,:,:,qi) 
				!
				!THOPP
				
				tImag(:,:,R)	= tImag(:,:,R)		+ phase * Htmp(:,:,qi)
			end do
		end do
		!
		!EXP VALUES ARE REAL
...
!private
	...
	subroutine calcHtmp(EnH, Uq, Htmp)
	...
	subroutine testIfReal(rImag, tImag)
		complex(dp),		intent(in)		:: rImag(:,:,:,:), tImag(:,:,:)
