module ham_Magnetic
	...
	subroutine add_magHam(nG_qi, Gvec, Hmat)
		...
		complex(dp)						::	H_prefact, yInt
		!
		!period of oscillating B field
		qX_period	=	2.0_dp * PI_dp / aX 
		H_prefact 	= 	dcmplx( 	Bext(3) * aX / (4.0_dp*qX_period*vol)	)		!purely imaginary due to i in formula
		!
		!Loop elements
		count=0
		do j = 1, nG_qi
			do i = 1, nG_qi
				...
				if( i/=j ) then
					...
					if(  abs(dGy) > machineP ) then
						!
						yInt		= myExp(dGy*aY) - 1.0_dp
						!
						!
						if( abs(dGx-abs(qX_period))	< 1e-7_dp	) then
							!
							!case1: dGx == + qX_period
							if( 				abs(dGx-qX_period )	< 		1e-7_dp				) 				then
								!
								Hmat(i,j)	= Hmat(i,j) + H_prefact * yInt
								count = count + 1
								!
								!
							!case2: dGx == - qX_period
							else if(			abs(dGx+qX_period )	< 		1e-7_dp				)				 then
								Hmat(i,j)	=	Hmat(i,j) - H_prefact * yInt
								count = count + 1
								!
								!
		...
		!
		if( count <= 0) write(*,'(a,i3,a)') "[#",myID,";addMagHam]: WARNING dGx = |q_x| condition was never matched"	
		write(*,'(a,i3,a,i7,a,i7)') "[#",myID,";addMagHam]: dGx = |q_x| was mathed ",count," times"
		return
	end subroutine
...
!old version with wrong matrix elements implemented
!	subroutine add_magHam(nG_qi, Gvec, Hmat)
!		!adds oscillating magnetic field to hamiltonian via peierls sub
!		!
!		!
